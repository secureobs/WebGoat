...
| `SECUREOBS_PROJECT_ID` | secret | Canary project GUID (project → Integration panel) | **TODO — set this** |
| `SECUREOBS_TENANT_ID` | secret | Canary tenant GUID (Settings → Organization) | **TODO — set this** |
...
`secureobs-canary.yml` via the GitHub REST API with a unique `pipeline-run-id`
input, then polls the SecureObs API for that run id to confirm findings were
ingested and the gate reports blocking. See
